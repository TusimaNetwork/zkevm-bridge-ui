...
export const formatFiatAmount = (value: BigNumber): string => {
  ...
  const trimmed = decimals.length > FIAT_DISPLAY_PRECISION ? decimals.slice(0, FIAT_DISPLAY_PRECISION) : decimals
  return trimmed === "" || trimmed === "0" ? whole : `${whole}.${trimmed}`
}
...
export const multiplyAmounts = (a: Amount, b: Amount, outputPrecision: number): BigNumber => {
  ...
  const trimmedDecimals = decimals.length > outputPrecision ? decimals.slice(0, outputPrecision) : decimals;
